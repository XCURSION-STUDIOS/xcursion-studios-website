...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="page-body" style={{ padding: mobile ? '80px 0 40px' : '100px 0 80px' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: mobile ? '0 20px' : '0 56px' }}>

        {/* Back link */}
        <div style={{ marginBottom: '32px' }}>
          <Link href="/shop" style={{ fontSize: '9px', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'rgba(240,230,208,0.5)', textDecoration: 'none' }}>← All Items</Link>
        </div>
        ...
        {mobile ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', marginBottom: '48px' }}>
            {/* Main image */}
            <div style={{ aspectRatio: '4/5', background: product.imageUrl ? `url(${product.imageUrl}) center/cover no-repeat` : 'var(--bg)', position: 'relative' }}>
              <div style={{ position: 'absolute', top: '16px', left: '16px', fontSize: '8px', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(240,230,208,0.8)', border: '1px solid rgba(240,230,208,0.2)', padding: '3px 10px', background: 'rgba(7,6,13,0.6)' }}>{product.badge}</div>
            </div>
            {/* Title + price */}
            <div>
              <div style={{ fontSize: '8px', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '10px' }}>{product.category}</div>
              <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 300, fontSize: 'clamp(28px,7vw,44px)', lineHeight: 0.95, color: 'var(--cream)', marginBottom: '12px' }}>{product.name}</h1>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '28px', fontWeight: 300, color: 'rgba(240,230,208,0.85)', marginBottom: '20px', borderBottom: '1px solid rgba(240,230,208,0.08)', paddingBottom: '20px' }}>{product.price}</div>
              <p style={{ fontSize: '13px', color: 'rgba(240,230,208,0.6)', lineHeight: 1.8 }}>{product.description}</p>
            </div>
            {/* Specs */}
            {product.specs?.length > 0 && (
              <div>
                <div style={{ fontSize: '8px', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(240,230,208,0.35)', marginBottom: '16px' }}>Details</div>
                {product.specs.map((s: any) => (
                  <div key={s._key} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid rgba(240,230,208,0.08)', fontSize: '11px' }}>
            ...
            {!submitted ? (
              <div>
                <div style={{ fontSize: '8px', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(240,230,208,0.35)', marginBottom: '16px' }}>Inquire to Buy</div>
                ...
                <button onClick={handleSubmit} style={{ width: '100%', padding: '14px', background: 'transparent', border: '1px solid rgba(240,230,208,0.2)', color: 'var(--cream)', fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.25em', textTransform: 'uppercase', cursor: 'pointer' }}>
                  Send Inquiry →
                </button>
              </div>
            ) : (
          ...
          /* Desktop layout — grid */
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 1fr', gridTemplateRows: 'auto auto auto', columnGap: '48px', rowGap: '80px', marginBottom: '80px' }}>
        ...
        {other && (
          <div style={{ marginTop: mobile ? '0' : '0' }}>
            ...
            <Link href={`/shop/${other.slug.current}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: mobile ? '20px' : '28px', border: '1px solid rgba(240,230,208,0.1)', textDecoration: 'none', color: 'inherit', cursor: 'auto' }}>
              <div>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: mobile ? '16px' : '20px', color: 'var(--cream)', fontWeight: 300, marginBottom: '4px' }}>{other.name}</div>
                <div style={{ fontSize: '10px', color: 'rgba(240,230,208,0.45)', letterSpacing: '0.1em' }}>{other.category}</div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: mobile ? '18px' : '22px', color: 'var(--cream)', fontWeight: 300 }}>{other.price}</div>
