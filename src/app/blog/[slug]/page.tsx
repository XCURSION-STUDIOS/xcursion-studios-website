  ...
  return (
    <main className="page-body" style={{ padding: 0 }}>
      <div style={{ position: 'relative', width: '100%', display: 'flex', alignItems: 'flex-end', overflow: 'hidden', paddingTop: mobile ? '80px' : '120px' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: mobile ? '0 20px 40px' : '0 56px 64px', maxWidth: '900px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 300, fontSize: mobile ? 'clamp(28px,7vw,48px)' : 'clamp(36px,5.5vw,80px)', lineHeight: 1.0, letterSpacing: '-0.02em', color: 'var(--cream)', marginBottom: '24px' }}>{post.title}</h1>
          <div style={{ display: 'flex', gap: '24px', fontSize: '10px', color: 'rgba(240,230,208,0.7)', letterSpacing: '0.15em' }}>
            <span>{post.date}</span><span>{post.readTime}</span>
          </div>
        </div>
      </div>
      {post.imageUrl && <div style={{ width: '100%', maxHeight: mobile ? '280px' : '560px', overflow: 'hidden' }}><img src={post.imageUrl} alt={post.title} style={{ width: '100%', height: mobile ? '280px' : '560px', objectFit: 'cover', objectPosition: 'center', display: 'block', opacity: 0.9 }} /></div>}
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: mobile ? '40px 20px 80px' : '80px 56px 120px', fontFamily: 'var(--font-display)', fontSize: mobile ? '16px' : '19px', fontWeight: 300, lineHeight: 1.9, color: 'var(--cream)' }}>
        {post.excerpt}
      </div>
      <nav style={{ borderTop: '1px solid rgba(201,168,76,0.2)', padding: mobile ? '32px 20px' : '48px 56px', display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center' }}>
        {prev ? <Link href={`/blog/${prev.slug.current}`} style={{ textDecoration: 'none', color: 'var(--cream)', cursor: 'auto' }}>
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontSize: mobile ? '13px' : '16px', fontWeight: 300 }}>{prev.title}</div>
        </Link> : <div />}
        <Link href="/blog" style={{ fontSize: '10px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(240,230,208,0.7)', textDecoration: 'none', cursor: 'auto', textAlign: 'center' }}>All Posts</Link>
        {next ? <Link href={`/blog/${next.slug.current}`} style={{ textDecoration: 'none', color: 'var(--cream)', cursor: 'auto', textAlign: 'right', justifySelf: 'end' }}>
          <div style={{ fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(240,230,208,0.7)', marginBottom: '6px' }}>Next →</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: mobile ? '13px' : '16px', fontWeight: 300 }}>{next.title}</div>
        </Link> : <div />}
      </nav>
    </main>
  );
