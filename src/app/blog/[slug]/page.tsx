...
export default function PostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!post) return <main className="page-body" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh' }}><div style={{ color: 'var(--cream-faint)', fontSize: '11px', letterSpacing: '0.2em' }}>Loading...</div></main>;
    ...
    <main className="page-body">
      <div style={{ position: 'relative', width: '100%', height: '70vh', minHeight: '480px', display: 'flex', alignItems: 'flex-end', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: '0 56px 64px', maxWidth: '900px' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 300, fontSize: 'clamp(36px,5.5vw,80px)', lineHeight: 1.0, letterSpacing: '-0.02em', color: 'var(--white)', marginBottom: '24px' }}>{post.title}</h1>
          <div style={{ display: 'flex', gap: '24px', fontSize: '10px', color: 'var(--cream-faint)', letterSpacing: '0.15em' }}>
            <span>{post.date}</span><span>{post.readTime}</span>
          </div>
        </div>
      </div>
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: '80px 56px 120px', fontFamily: 'var(--font-display)', fontSize: '19px', fontWeight: 300, lineHeight: 1.9, color: 'var(--cream-dim)' }}>
        {post.excerpt}
      </div>
      <nav style={{ borderTop: '1px solid var(--cream-faint)', padding: '48px 56px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        {prev ? <Link href={`/blog/${prev.slug.current}`} style={{ textDecoration: 'none', color: 'var(--cream-dim)', cursor: 'none' }}>
        ...
        {next ? <Link href={`/blog/${next.slug.current}`} style={{ textDecoration: 'none', color: 'var(--cream-dim)', cursor: 'none', textAlign: 'right' }}>
